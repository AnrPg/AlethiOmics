...
# ──────────────────────────────────────────────────────────────────────────
#  Dumb fetchers that return mock data - for speed and debug reasons
# ──────────────────────────────────────────────────────────────────────────
...
_TRANSFORM_REGISTRY: Dict[str, Callable[..., Any]] = {
    **SIMPLE_REGISTRY,
    ...
    "stub_fetch_gene_metadata": stub_fetch_gene_metadata,
    "stub_fetch_stimulus_metadata": stub_fetch_stimulus_metadata,
    "stub_fetch_microbe_metadata": stub_fetch_microbe_metadata,
    "stub_fetch_taxon_metadata": stub_fetch_taxon_metadata,
    "stub_fetch_ontology_term_metadata": stub_fetch_ontology_term_metadata,
    "stub_fetch_study_metadata": stub_fetch_study_metadata,
    "stub_fetch_sample_metadata": stub_fetch_sample_metadata
}
...
class Harmonizer:
    ...
    def apply(self, table: str, rows: Iterable[Dict]) -> List[Dict]:
        ...
        for i, row in enumerate(rows):
            # logger.debug(f"ROW {i}: starting harmonization for row id={row.get('sample_id', '<no id>')}")
            new_row = dict(row)  # shallow copy

            # Run every rule on every original value in the input row
            for val in list(row.values()):
                # logger.debug(f"ROW {i}:   examining value={val!r}")
